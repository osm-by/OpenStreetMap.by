...
function context_menu(e){
  ...
  if (routePoints.length >= 1) {
      ...
      var item = {};
      item[(vehicle!="bike"?'☐ ':'☑ ')+ _("Route as bicycle")] = {onclick:function(menuItemClicked,menuObject) { vehicle="bike"; updateRoute(); }, icon:"/static/img/bicycle_16.png", disabled: vehicle=="bike"};
      menu.push(item);

      var item = {};
      item[(vehicle!="car"?'☐ ':'☑ ')+ _("Route as car")] = {onclick:function(menuItemClicked,menuObject) { vehicle="car"; updateRoute(); }, icon:"/static/img/wheel_16.png", disabled: vehicle=="car"};
      menu.push(item);
      if (canRouteTaxi()){
        var item = {};
        item[(vehicle!="taxi"?'☐ ':'☑ ')+ _("Route as taxi")] = {onclick:function(menuItemClicked,menuObject) { vehicle="taxi"; updateRoute(); }, icon:"/static/img/taxi_c.png", disabled: vehicle=="taxi"};
        menu.push(item);
      }
      var item = {};
      item[ _("Reverse route")] = {onclick:function(menuItemClicked,menuObject) { routePoints.reverse(); updateRoute(); }, icon:"/static/img/reverse.png"};
      menu.push(item);
      
      var item = {};
      item[ _("Clear route")] = {onclick:function(menuItemClicked,menuObject) {from_geolocation = true; routePoints = []; updateRoute(); }, icon:"/static/img/delete_icon.gif"};
      menu.push(item);
  }
  ...
  var item = {};
  item[ (_("Report a problem") + ((mmap.getZoom()<13)?(" ("+ _("please zoom in")+")"):''))] = {onclick:OSBmenu, icon:"/static/img/osb_icon.png", disabled: mmap.getZoom()<13};
  menu.push(item);
  ...
  if (josm_remote){
    ...
    item[_("Edit via ")+josm_remote.application ] = {
      ...
      icon: '/static/img/icon_to_josm.png'
    }
    menu.push(item);
  }else{
    var item = {};
